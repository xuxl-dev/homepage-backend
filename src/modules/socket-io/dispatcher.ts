...
} from '../internal-message/internal-message.service';
  ...
  process: (msg: Message) => Promise<Message> = async (msg: Message) => {
    if (isValidACK(msg)) {
      ...
      if (this.pendingMessages.has(ackMsgId)) {
        // at least one ack means the message is delivered successfully
        this.pendingMessages.get(ackMsgId)();
        this.pendingMessages.delete(ackMsgId);
      }
    } else {
      if (
        isFlagSet(msg, MessageFlag.DO_NOT_STORE) || // just let message fail, don't store it
      ...
      const messageFail = setTimeout(() => {
        console.log(
          `online message ${msg.msgId} failed, fall back to offline message`,
        );
...
export class Dispatcher {
  ...
  async castMessage(message: Message) {
    ...
    if (isFlagSet(message, MessageFlag.BROADCAST)) {
      // TODO: clean this, use strategy pattern

      // TODO: this will only send to online users,
      // offline users will not receive this message
      this.io.to(`group:${message.receiverId}`).emit(messageToken, message);
